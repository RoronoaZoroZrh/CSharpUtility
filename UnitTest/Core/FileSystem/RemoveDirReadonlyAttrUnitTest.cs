...
namespace UnitTest
{
    public class RemoveDirReadonlyAttrUnitTest : BaseUnitTest
    {
        //!运行测试用例
        public override Boolean RunUnitTest()
        {
            ...
            //!调用接口，正确情况
            CSharpUtility.Helper.RemoveDirReadonlyAttr(sTestDir1);
            Boolean bExpectResult = true;
            Boolean bActualResult = vFileInfo1.IsReadOnly == false && vFileInfo2.IsReadOnly == false;

            //!调用接口，传入非法参数
            CSharpUtility.Helper.RemoveDirReadonlyAttr(null);
            CSharpUtility.Helper.RemoveDirReadonlyAttr("");
